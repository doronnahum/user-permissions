...
describe('Test Ability class', () => {
  ...
  const createPost = new Ability({actions: 'create', subject: 'posts', conditions: {publish: true}});
  const createOnePost = new Ability({actions: 'create', subject: 'posts', allowOne: true});
  ...
  it('Validate read action + conditions', () => {
    expect(createPost.can('create', 'posts', undefined, {title: 'lorem', publish: true})).toEqual(true)
  });
  ...
  it('Validate read action + conditions - many when allowOne is true [should failed]', () => {
    expect(createOnePost.can('create', 'posts', undefined, [{title: 'lorem', publish: true}])).toEqual(false)
  });

  it('Validate read action + conditions [should failed]', () => {
    expect(createPost.can('create', 'posts', undefined, {title: 'lorem', publish: false})).toEqual(false)
  });
