...
const isStringOrArrayOfString = (res: any) => typeof res === 'string' || Array.isArray(res) && !res.some(item => typeof item !== 'string');
export const validateActions = (actions: Actions) => {
  if (!isStringOrArrayOfString(actions)) {
    throw new Error('check-abilities: actions is required and must be string | string[]');
  }
  return true;
};
...
export const validateFields = (fields: string | Fields) => {
  if (!isStringOrArrayOfString(fields)) {
    throw new Error('check-abilities: fields must be type of string[] or string');
  }
  return true;
};
